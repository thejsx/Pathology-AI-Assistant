...
export default function VideoPlayer({ streamRef }) {
    ...
    const applyTransform = () => {
        const targetElement = document.fullscreenElement ? wrapperRef.current : streamRef.current;
        if (!targetElement) return;
        targetElement.current.style.transform = `
    ...
    return (
        <div className="stream-controls">   
            <div ref={wrapperRef} style={{ width: '100%', height: '100%', overflow: 'hidden' }}>
                <video ref={streamRef} style={{ width: '100%', height: '100%' }} />
            </div>   

...
def is_ignored(spec: Optional[pathspec.PathSpec], rel_path: Path) -> bool:
    print(f"Checking if ignored: {rel_path}")
    print('rel_path:', rel_path.as_posix())
    print('spec file match:', spec.match_file(rel_path.as_posix()))
    print(bool(spec and spec.match_file(rel_path.as_posix())))
    print('---')
...
def should_skip_dir(name: str) -> bool:
    print(f"Checking if skip dir: {name}")
    print(name in SKIP_DIRS)
    print('---')
...
def is_probably_binary(path: Path, sniff_bytes: int = 2048) -> bool:
    ...
    if b'\x00' in chunk:
        print(f"Binary detected in {path}: contains NUL byte, skipping.")
    return b'\x00' in chunk    # NUL anywhere → call it binary
...
def print_contents(root: Path, buf: io.StringIO, spec: Optional[pathspec.PathSpec]):
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        dirnames[:] = [d for d in dirnames if not should_skip_dir(d) and not is_ignored(spec, Path(dirpath, d).relative_to(root))]
        print('Filtered dirnames:', dirnames)
        for fname in sorted(filenames):
            path = Path(dirpath) / fname
            rel   = path.relative_to(root)
            if is_ignored(spec, rel) or should_skip_file(path) or is_probably_binary(path):
                print(f"Skipping ignored file: {rel}")
                continue

            buf.write(f"{'='*80}\nFile: {rel.as_posix()}\n{'='*80}\n")
            print(f"Processing file: {rel.as_posix()}")
            try:
                if path.stat().st_size > max_bytes:
                    buf.write(f"[skipped: {path.stat().st_size//1024} KiB > {MAX_FILE_KB} KiB limit]\n\n")
                    print(f"Skipped: {rel} (size: {path.stat().st_size//1024} KiB > {MAX_FILE_KB} KiB limit)")
                    continue
